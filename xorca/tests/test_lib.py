...
def test_trim_and_sqeeze(model_config_and_trimming):
    ...
    x_sel = model_config_and_trimming["x"]
    y_sel = model_config_and_trimming["y"]
    ds_t = trim_and_squeeze(ds, model_config=model_config)
    ds_trimmed_here = ds.isel(x=slice(*x_sel), y=slice(*y_sel))
